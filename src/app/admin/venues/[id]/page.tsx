...
export default async function VenueDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ponytail: owner_id has no unique constraint, so supabase-js's generated
  // types treat the embed as one-to-many; PostgREST still returns a single
  // object for this FK direction. Cast to the real runtime shape.
    ...
    <div className="flex flex-col gap-4">
      ...
      {venue.owner_id ? (
        <p className="text-sm text-accent">
          {t('ownerAssigned')} — {owner?.full_name} ({owner?.phone})
        </p>
      ) : (
        <form
